...
using SpendWise.Modules.Users.API.Users.Endpoints;
using SpendWise.Modules.Users.API.Users.Endpoints.User;
...
[Route("users")]
internal class UsersController(IDispatcher dispatcher) : ControllerBase
{
    [HttpPost("create")]
    ...
    public async Task<ActionResult<UserDetailsDto>> GetAsync(Guid userId)
        => Ok(await dispatcher.QueryAsync(new GetUserQuery(userId)));
