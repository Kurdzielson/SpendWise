...
namespace SpendWise.Shared.Infrastructure.Contracts;

internal sealed class ContractRegistry(IModuleRegistry moduleRegistry, ILogger<ContractRegistry> logger)
    : IContractRegistry
{
    ...
    private void ValidatePaths()
    {
        foreach (var (path, (requestType, responseType)) in _paths)
        {
            ...
            logger.LogTrace($"Validating the contracts for path: '{path}'...");
            ...
            logger.LogTrace($"Validated the contracts for path: '{path}'.");
        }
    }
    ...
    private void ValidateContract(Type contractType, string path = null)
    {
        ...
        logger.LogTrace($"Validating the contract for: '{contractName}', " +
        ...
        logger.LogTrace($"Successfully validated the contract for: '{contractName}', " +
                         $"from module: '{contractModule}', original module: '{module}'.");
    }
